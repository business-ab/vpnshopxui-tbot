...
if($data=="inviteSetting" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        ...
        [
            ...
            ['text'=>"مقدار پورسانت",'callback_data'=>"VPNFREE_4"]
            ],
        [
...
🌐 subscription : <code>$subLink</code>
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"بنازم خرید جدید ❤️",'callback_data'=>"VPNFREE_4"]
        ],
        ]]);
...
$keys = json_encode(['inline_keyboard'=>[
    [
        ['text'=>"به به تمدید 😍",'callback_data'=>"VPNFREE_4"]
        ],
    ]]);
...
if($response->success){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"اخیش یکی زمان زد 😁",'callback_data'=>"VPNFREE_4"]
            ],
        ]]);
sendMessage("
...
if($response->success){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"اخیش یکی حجم زد 😁",'callback_data'=>"VPNFREE_4"]
            ],
        ]]);
sendMessage("
...
elseif($payType == "RENEW_SCONFIG"){
    ...
    editKeys(json_encode(['inline_keyboard'=>[
		    [['text'=>"پرداخت انجام شد",'callback_data'=>"VPNFREE_4"]]
		    ]]));
}else{
    ...
    if(preg_match('/^discountCustomPlanDay/', $userInfo['step'])){
        ...
        if($list->num_rows>0){
            ...
            if($canUse > $userUsedCount){
                ...
                $keys = json_encode(['inline_keyboard'=>[
                    [
                        ['text'=>"❤️", "callback_data"=>"VPNFREE_4"]
                        ],
                    ]]);
            sendMessage(
...
\n🌐 subscription : <code>$subLink</code>";
    ...
    unset($markup[count($markup)-1]);
    $markup[] = [['text'=>"✅",'callback_data'=>"VPNFREE_4"]];
    $keys = json_encode(['inline_keyboard'=>array_values($markup)],488);
    ...
    if($admin != $from_id){ 
        $keys = json_encode(['inline_keyboard'=>[
            [
                ['text'=>"به به 🛍",'callback_data'=>"VPNFREE_4"]
            ],
            ]]);
...
if($data=="availableServers"){
    ...
    $keys = array();
    $keys[] = [
        ['text'=>"تعداد باقیمانده",'callback_data'=>"VPNFREE_4"],
        ['text'=>"پلن",'callback_data'=>"VPNFREE_4"],
        ['text'=>'سرور','callback_data'=>"VPNFREE_4"]
        ];
    while($file_detail = $serversList->fetch_assoc()){
        ...
        if($name->num_rows>0){
            ...
            $keys[] = [
                ['text'=>$acount . " اکانت",'callback_data'=>"VPNFREE_4"],
...
if($data=="availableServers2"){
    ...
    while($file_detail2 = $serversList->fetch_assoc()){
        ...
        if($name->num_rows>0){
            ...
            $keys[] = [
                ['text'=>$acount2 . " اکانت",'callback_data'=>"VPNFREE_4"],
                ['text'=>$title2??" ",'callback_data'=>"VPNFREE_4"],
                ];
        }
    }
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"mainMenu"]];
...
if(preg_match('/^agencyDecline(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editKeys(json_encode(['inline_keyboard'=>[
        [['text'=>$buttonValues['declined'],'callback_data'=>"VPNFREE_4"]]
        ]]));
...
if(preg_match('/^agencyApprove(\d+)_(\d+)/',$userInfo['step'],$match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if(is_numeric($text)){
        editKeys(json_encode(['inline_keyboard'=>[
            [['text'=>$buttonValues['approved'],'callback_data'=>"VPNFREE_4"]]
            ]]), $match[2]);
...
if(preg_match('/decline(\d+)_(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) and $text != $buttonValues['cancel']){
    ...
    editKeys(
	    ...
	    [['text'=>"لغو شد ❌",'callback_data'=>"VPNFREE_4"]]
	    ]]) ,$match[2]);
...
if($data == 'dayPlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [];
    $keyboard[] = [['text'=>"حذف",'callback_data'=>"VPNFREE_4"],['text'=>"قیمت",'callback_data'=>"VPNFREE_4"],['text'=>"تعداد روز",'callback_data'=>"VPNFREE_4"]];
...
if($data == 'volumePlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [];
    $keyboard[] = [['text'=>"حذف",'callback_data'=>"VPNFREE_4"],['text'=>"قیمت",'callback_data'=>"VPNFREE_4"],['text'=>"مقدار حجم",'callback_data'=>"VPNFREE_4"]];
...
if(preg_match('/^closeTicket_(\d+)/',$data,$match) and  $from_id != $admin){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"$from_id",'callback_data'=>"VPNFREE_4"],
            ['text'=>"آیدی کاربر",'callback_data'=>'VPNFREE_4']
        ],
        [
            ['text'=>$first_name??" ",'callback_data'=>"VPNFREE_4"],
            ['text'=>"اسم کاربر",'callback_data'=>'VPNFREE_4']
        ],
        [
            ['text'=>"$title",'callback_data'=>'VPNFREE_4'],
            ['text'=>"عنوان",'callback_data'=>'VPNFREE_4']
        ],
        [
            ['text'=>"$category",'callback_data'=>'VPNFREE_4'],
            ['text'=>"دسته بندی",'callback_data'=>'VPNFREE_4']
        ],
        ]]);
...
if(preg_match("/^rate_+([0-9])+_+([0-9])/",$data,$match)){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"رای تیکت",'callback_data'=>"VPNFREE_4"]
            ],
        ]]);
...
if($data=='ticketsCategory' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $stmt->close();
    $keys = array();
    $keys[] = [['text'=>"حذف",'callback_data'=>"VPNFREE_4"],['text'=>"دسته بندی",'callback_data'=>"VPNFREE_4"]];
    
    if($ticketCategory->num_rows>0){
        while($row = $ticketCategory->fetch_assoc()){
            $rowId = $row['id'];
            $ticketName = $row['value'];
            $keys[] = [['text'=>"❌",'callback_data'=>"delTicketCat_$rowId"],['text'=>$ticketName,'callback_data'=>"VPNFREE_4"]];
        }
    }else{
...
if(preg_match('/^closeTicket_(\d+)/',$data,$match) and  ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    editKeys(json_encode(['inline_keyboard'=>[
        [['text'=>"تیکت بسته شد",'callback_data'=>"VPNFREE_4"]]
        ]]));
    ...
    while($row = $serversList->fetch_assoc()){
        ...
        if($response->success){
            ...
            if(strpos($list, $text)){
                ...
                $keys = json_encode(['inline_keyboard'=>[
                [
                    ['text'=>$state??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"🔘 وضعیت اکانت 🔘",'callback_data'=>"VPNFREE_4"],
                    ],
                [
					['text'=>$remark??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"« نام اکانت »",'callback_data'=>"VPNFREE_4"],
                    ],
                [
                    ['text'=>$upload?? " ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"√ آپلود √",'callback_data'=>"VPNFREE_4"],
                    ],
                [
                    ['text'=>$download??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"√ دانلود √",'callback_data'=>"VPNFREE_4"],
                    ],
                [
                    ['text'=>$total??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"† حجم کلی †",'callback_data'=>"VPNFREE_4"],
                    ],
                [
                    ['text'=>$leftMb??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"~ حجم باقیمانده ~",'callback_data'=>"VPNFREE_4"],
                    ],
                [
                    ['text'=>$expiryTime??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"تاریخ اتمام",'callback_data'=>"VPNFREE_4"],
                    ],
                [
                    ['text'=>$expiryDay??" ",'callback_data'=>"VPNFREE_4"],
                    ['text'=>"تعداد روز باقیمانده",'callback_data'=>"VPNFREE_4"],
                    ],
...
if($data == 'backplan' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard[] = [['text'=>"➖➖➖",'callback_data'=>"VPNFREE_4"]];
...
if(($data=="editCustomPlan" || preg_match('/^editCustom(gbPrice|dayPrice)/',$userInfo['step'],$match)) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ...
            ['text'=>"هزینه هر گیگ",'callback_data'=>"VPNFREE_4"]
            ],
        [
            ['text'=>$dayPrice,'callback_data'=>"editCustomdayPrice"],
            ['text'=>"هزینه هر روز",'callback_data'=>"VPNFREE_4"]
            ],
        [
            ['text'=>$buttonValues['back_button'],'callback_data'=>"backplan"]
            ]
...
if(preg_match('/^vpnshopplanacclist(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^vpnshopplandelete(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^vpnshopplanname(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🔅 یه اسم برا پلن جدید انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^vpnshopplanname(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^vpnshopplanslimit(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🔅 ظرفیت جدید برای پلن انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^vpnshopplanslimit(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^vpnshopplansinobundid(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🔅 سطر جدید برای پلن انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^vpnshopplansinobundid(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^vpnshopplaneditdes(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🎯 توضیحاتت رو برام وارد کن:",$cancelKey);exit;
}
if(preg_match('/^vpnshopplaneditdes(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^vpnshopplanrial(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🎯 شیطون قیمت و گرون کردی 😂 ، خب قیمت جدید و بزن ببینم :",$cancelKey);exit;
}
if(preg_match('/^vpnshopplanrial(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)&& $text != $buttonValues['cancel']){
    if(is_numeric($text)){
...
if(preg_match('/^vpnshopcategorydelete(\d+)_(\d+)/',$data, $match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^vpnshopcategoryedit/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("〽️ یه اسم جدید برا دسته بندی انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/vpnshopcategoryedit(\d+)_(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^vpnshopdeleteserver(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
